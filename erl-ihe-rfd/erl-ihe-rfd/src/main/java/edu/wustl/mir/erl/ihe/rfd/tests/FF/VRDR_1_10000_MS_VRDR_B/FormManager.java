 ...
 *******************************************************************************/
package edu.wustl.mir.erl.ihe.rfd.tests.FF.VRDR_1_10000_MS_VRDR_B;
...
/**
 * FormManager simulator test for VRDR_1_10000_MS_VRDR_B
 */
public class FormManager extends FormManagerTest {
   private static final long serialVersionUID = 1L;

   private static final String TEST_ID = "VRDR_1_10000_MS_VRDR_B";
   ...
   public RetrieveFormResponseType formManagerRetrieveForm(
      ...
      ts = TSTStatus.TST_PREPOP_MS_VRDR_PASS;
      if (VRDRPrepop.getPrepopDocumentType(prepopData) 
          != VRDRPrepop.PREPOP_DOCUMENT_TYPE.MS_VRDR)      
         ts = TSTStatus.TST_PREPOP_MS_VRDR_FAIL;
      wsLog.addStatus(ts);
      valid = ts.getResult().overall(valid);

      ts = TSTStatus.TST_PREPOP_TIME_OF_DEATH_PASS;
      if(!vrdr.getTimeofDeath().getValue().startsWith("201507061330"))
         ts = TSTStatus.TST_PREPOP_TIME_OF_DEATH_FAIL;
      wsLog.addStatus(ts);
      valid = ts.getResult().overall(valid);
      
      ts = TSTStatus.TST_PREPOP_SEX_PASS;
      String sex = vrdr.getSex().getCode();
      if (sex.equalsIgnoreCase("M") == false)
   ...
   public enum TSTStatus implements Status {
      ...
      TST_PREPOP_MS_VRDR_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data is CDA MS-VRDR document: FAIL"),

      TST_PREPOP_MS_VRDR_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data is CDA MS-VRDR document: PASS"),
         
      TST_PREPOP_TIME_OF_DEATH_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data Time of Death is '7/6/2015 1:30PM': FAIL"),

      TST_PREPOP_TIME_OF_DEATH_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data Time of Death is '7/6/2015 1:30PM': PASS"),
         
      TST_PREPOP_SEX_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data sex code is 'M': FAIL"),

      TST_PREPOP_SEX_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data sex code is 'M': PASS"),
