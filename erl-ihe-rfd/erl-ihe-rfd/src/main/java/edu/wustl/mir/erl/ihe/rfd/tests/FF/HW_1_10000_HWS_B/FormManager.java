...
import edu.wustl.mir.erl.ihe.rfd.cda.document.HWS;
...
/**
 * FormManager simulator test for HW_1_10000_HWS_B
 */
public class FormManager extends FormManagerTest {
   ...
   public RetrieveFormResponseType formManagerRetrieveForm(
      ...
      TSTStatus ts = TSTStatus.TST_PREPOP_HWS_PASS;
      prepopData: {
         if (ppd == null) {
            ts = TSTStatus.TST_PREPOP_HWS_FAIL;
            wsLog.addStatus(ts);
         ...
         Element prepopData = any.get(0);
         try {
            hws.loadValue(prepopData);
         } catch (Exception e) {
            ts = TSTStatus.TST_PREPOP_HWS_FAIL;
            break prepopData;
         }
         
         // TODO Code here to verify that prepopData is valid CDA HWS Document
         
         ts = TSTStatus.TST_PREPOP_RACE_PASS;
         String race = hws.getRace().getCode();
         if (race.equalsIgnoreCase("2054-5") == false)
            ts = TSTStatus.TST_PREPOP_RACE_FAIL;
         wsLog.addStatus(ts);
         valid = ts.getResult().overall(valid);
         
         ts = TSTStatus.TST_PREPOP_SEX_PASS;
         String sex = hws.getSex().getCode();
         if (sex.equalsIgnoreCase("F") == false)
            ts = TSTStatus.TST_PREPOP_SEX_FAIL;
         wsLog.addStatus(ts);
         valid = ts.getResult().overall(valid);
      ...
      LoadNameValuePairs.retrieveFormRequest(body, wsLog, hws);
   ...
   public enum TSTStatus implements Status {
      ...
      TST_PREPOP_HWS_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data is CDA HWS document: FAIL"),

      TST_PREPOP_HWS_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data is CDA HWS document: PASS"),
         
      TST_PREPOP_RACE_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data race code is '2054-5': FAIL"),

      TST_PREPOP_RACE_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data race code is '2054-5': PASS"),
         
      TST_PREPOP_SEX_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "pre-population data sex code is 'F': FAIL"),

      TST_PREPOP_SEX_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "pre-population data sex code is 'F': PASS"),

      TST_FORMID_FAIL(Phase.TST, Result.FAIL, Level.DEBUG,
         "workflowData.formId is " + TEST_ID + ": FAIL"),

      TST_FORMID_PASS(Phase.TST, Result.PASS, Level.TRACE,
         "workflowData.formId is " + TEST_ID + ": PASS"),
